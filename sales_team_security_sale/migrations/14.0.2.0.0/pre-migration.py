...
def migrate(cr, version):
    ...
    if record := env.ref(
        "sales_team_security_sale.sale_order_team_rule", False
    ):
        record.domain_force = (
    ...
    if record := env.ref(
        "sales_team_security_sale.sale_order_report_team_rule", False
    ):
        record.domain_force = (
            "['|', '|', ('user_id','=',user.id), ('user_id','=',False), '|', "
            "('team_id', '=', user.sale_team_id.id), ('team_id', '=', False)]"
    ...
    if record := env.ref(
        "sales_team_security_sale.sale_order_line_team_rule", False
    ):
        record.domain_force = (
